...
#define INITCOUNTERLIST(threadId, nodeId, folder, startPoint)                  \
  artsInitCounterList(threadId, nodeId, folder, startPoint)
...
#define ARTSCREATECOUNTER(counter)                                             \
  artsCreateCounter(threadId, nodeId, counterName)
#define ARTSGETCOUNTER(counter) artsGetCounter(counter)

#define ARTSCOUNTERINCREMENT(counter)                                          \
  IF(COUNTER_ON(counter))(artsCounterIncrement, EAT)(artsGetCounter(counter))
#define ARTSCOUNTERINCREMENTBY(counter, num)                                   \
  IF(COUNTER_ON(counter))(artsCounterIncrementBy,                              \
                          EAT)(artsGetCounter(counter), num)
#define ARTSCOUNTERTIMERSTART(counter)                                         \
  IF(COUNTER_ON(counter))(artsCounterTimerStart, EAT)(artsGetCounter(counter))
#define ARTSCOUNTERTIMERENDINCREMENT(counter)                                  \
  IF(COUNTER_ON(counter))(artsCounterTimerEndIncrement,                        \
                          EAT)(artsGetCounter(counter))
#define ARTSCOUNTERTIMERENDINCREMENTBY(counter, num)                           \
  IF(COUNTER_ON(counter))(artsCounterTimerEndIncrementBy,                      \
                          EAT)(artsGetCounter(counter), num)
#define ARTSCOUNTERADDTIME(counter, time)                                      \
  IF(COUNTER_ON(counter))(artsCounterAddTime, EAT)(artsGetCounter(counter),    \
                                                   time)
#define ARTSCOUNTERSETENDTOTIME(counter)                                       \
  IF(COUNTER_ON(counter))(artsCounterSetEndTime, EAT)(artsGetCounter(counter), \
                                                      artsExtGetTimeStamp())
#define ARTSCOUNTERADDENDTIME(counter)                                         \
  IF(COUNTER_ON(counter))(artsCounterAddEndTime, EAT)(artsGetCounter(counter))
#define ARTSCOUNTERNONEMPTY(counter)                                           \
  IF(COUNTER_ON(counter))(artsCounterNonEmtpy, EAT)(artsGetCounter(counter))

#define INTERNAL_ARTSEDTCOUNTERTIMERSTART(counter)                             \
  artsCounterTimerStart(artsGetCounter(                                        \
      (artsThreadInfo.currentEdtGuid) ? (counter##On) : (counter)))
#define ARTSEDTCOUNTERTIMERSTART(counter)                                      \
  IF(COUNTER_ON(counter))(INTERNAL_ARTSEDTCOUNTERTIMERSTART, EAT)(counter)

#define INTERNAL_ARTSEDTCOUNTERTIMERENDINCREMENT(counter)                      \
  artsCounterTimerEndIncrement(artsGetCounter(                                 \
      (artsThreadInfo.currentEdtGuid) ? (counter##On) : (counter)))
#define ARTSEDTCOUNTERTIMERENDINCREMENT(counter)                               \
  IF(COUNTER_ON(counter))(INTERNAL_ARTSEDTCOUNTERTIMERENDINCREMENT,            \
                          EAT)(counter)
...
#define ARTSUSERCOUNTERTIMERSTART(counter)                                     \
  artsCounterTimerStart(artsUserGetCounter(counter, #counter))
#define ARTSUSERCOUNTERTIMERENDINCREMENT(counter)                              \
  artsCounterTimerEndIncrement(artsUserGetCounter(counter, #counter))
#define USERCOUNTERS(first, ...)                                               \
  enum __userCounters { first = lastCounter, __VA_ARGS__ }
...
#define ARTSCOUNTERINCREMENT(counter)                                          \
  artsCounterIncrement(artsGetCounter(counter))
#define ARTSCOUNTERINCREMENTBY(counter, num)                                   \
  artsCounterIncrementBy(artsGetCounter(counter), num)
#define ARTSCOUNTERTIMERSTART(counter)                                         \
  artsCounterTimerStart(artsGetCounter(counter))
#define ARTSCOUNTERTIMERENDINCREMENT(counter)                                  \
  artsCounterTimerEndIncrement(artsGetCounter(counter))
#define ARTSCOUNTERTIMERENDINCREMENTBY(counter, num)                           \
  artsCounterTimerEndIncrementBy(artsGetCounter(counter), num)
#define ARTSCOUNTERTIMERENDOVERWRITE(counter)                                  \
  artsCounterTimerEndOverwrite(artsGetCounter(counter))
#define ARTSCOUNTERPRINT(counter, stream)                                      \
  artsCounterPrint(artsGetCounter(counter), stream)
#define ARTSCOUNTERSETSTARTTIME(counter, start)                                \
  artsCounterSetStartTime(artsGetCounter(counter), start)
#define ARTSCOUNTERSETENDTIME(counter, end)                                    \
  artsCounterSetEndTime(artsGetCounter(counter), end)
#define ARTSCOUNTERGETSTARTTIME(counter)                                       \
  artsCounterGetStartTime(artsGetCounter(counter))
#define ARTSCOUNTERGETENDTIME(counter)                                         \
  artsCounterGetEndTime(artsGetCounter(counter))
#define ARTSCOUNTERADDTIME(counter, time)                                      \
  artsCounterAddTime(artsGetCounter(counter), time)
#define ARTSUSERGETCOUNTER(counter) artsUserGetCounter(counter, #counter)
#define ARTSCOUNTERSETENDTOTIME(counter)                                       \
  artsCounterSetEndTime(artsGetCounter(counter), artsExtGetTimeStamp())
#define ARTSCOUNTERADDENDTIME(counter)                                         \
  artsCounterAddEndTime(artsGetCounter(counter))
#define ARTSCOUNTERNONEMPTY(counter)                                           \
  artsCounterNonEmtpy(artsGetCounter(counter))

#define ARTSEDTCOUNTERINCREMENT(counter)                                       \
  artsCounterIncrement(artsGetCounter(                                         \
      (artsThreadInfo.currentEdtGuid) ? (counter##On) : (counter)))
#define ARTSEDTCOUNTERINCREMENTBY(counter, num)                                \
  artsCounterIncrementBy(artsGetCounter((artsThreadInfo.currentEdtGuid)        \
                                            ? (counter##On)                    \
                                            : (counter)),                      \
                         num)
#define ARTSEDTCOUNTERTIMERSTART(counter)                                      \
...
#define ARTSUSERCOUNTERINCREMENT(counter)                                      \
  artsCounterIncrement(artsUserGetCounter(counter, #counter))
#define ARTSUSERCOUNTERINCREMENTBY(counter, num)                               \
  artsCounterIncrementBy(artsUserGetCounter(counter, #counter), num)
#define ARTSUSERCOUNTERTIMERSTART(counter)                                     \
  artsCounterTimerStart(artsUserGetCounter(counter, #counter))
#define ARTSUSERCOUNTERTIMERENDINCREMENT(counter)                              \
  artsCounterTimerEndIncrement(artsUserGetCounter(counter, #counter))
...
#define COUNTERNAMES                                                           \
  const char *const __counterName[] = {"edtCounter",                           \
                                       ...
                                       "signalPersistentEventCounterOn",       \
                                       "signalEdtCounter",                     \
                                       "signalEdtCounterOn",                   \
                                       "edtCreateCounter",                     \
                                       "edtCreateCounterOn",                   \
                                       ...
                                       "persistentEventCreateCounterOn",       \
                                       "dbCreateCounter",                      \
                                       "dbCreateCounterOn",                    \
                                       "smartDbCreateCounter",                 \
                                       "smartDbCreateCounterOn",               \
                                       "mallocMemory",                         \
                                       "mallocMemoryOn",                       \
                                       "callocMemory",                         \
                                       "callocMemoryOn",                       \
