  ...
  if (inspFileName) {
    ...
    if (inspOutputPrefix && traceLevel < artsMETRICLEVELS) {
      if (traceLevel <= artsSystem) {
        ...
        inspectorShots->coreMetric =
            artsCalloc(sizeof(artsArrayList *) * artsLastMetricType *
                       artsNodeInfo.totalThreadCount);
        ...
        inspectorShots->nodeLock =
            artsCalloc(sizeof(unsigned int) * artsLastMetricType);
...
artsMetricLevel artsInternalUpdatePerformanceMetric(artsMetricType type,
  ...
  if (inspectorIgnore && type == artsThread)
    return artsInternalUpdatePerformanceCoreMetric(0, artsNode, level, toAdd,
                                                   sub);
