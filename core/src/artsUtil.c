...
// }
//
// void artsParallelStartRead(artsGuid_t * arrayOfGuids, unsigned int
// numberOfGuids,
...
//         {
//             if(blocksPerNode%artsNodeInfo.workerThreadCount ==
...
//                         struct ocrDb * ptr = ocrCalloc(sizeof(struct ocrDb) +
//                         dbSize); OCRSETMEMSHOTTYPE(ocrDefaultMemorySize);
//                         memcpy(ptr+1, dbTempPtr, dbSize);
//                         ocrFree(dbTempPtr);
//                         ocrDbCreateInternal((void*)ptr, dbSize, DB_PROP_NONE,
//                         NULL_GUID, sizeof(struct ocrDb)+dbSize, false);
//                         ocrRouteTableUpdateItem(ptr, arrayOfGuids[i],
//                         ocrGlobalRankId, 0);
//                         ocrRouteTableFireOO(arrayOfGuids[i],
//                         ocrOutOfOrderHandler);
//                     }
//                 }
//                 else
//                 {
...
// }
//
// void ocrParallelStartReadFixedSizeMMAP(ocrGuid_t * arrayOfGuids, unsigned int
// numberOfGuids,
//                               unsigned int size, ocrRecordReader_t reader,
//                               char * pathToFile)
//{
//     unsigned int blocksPerNode = 0;
//     for(unsigned int i=0; i<numberOfGuids; i++)
...
//         {
//             if(blocksPerNode%ocrNodeInfo.workerThreadCount ==
//             ocrThreadInfo.groupId)
//             {
//                 {
//                     OCRSETMEMSHOTTYPE(ocrDbMemorySize);
//                     struct ocrDb * ptr = ocrCalloc(sizeof(struct ocrDb) +
//                     size); OCRSETMEMSHOTTYPE(ocrDefaultMemorySize); void *
//                     temp = ptr + 1; unsigned int dbSize = size;
//                     reader(pathToFile, i, &dbSize, &temp);
//                     ocrDbCreateInternal((void*)ptr, dbSize, DB_PROP_NONE,
//                     NULL_GUID, sizeof(struct ocrDb) + size, false);
//                     ocrRouteTableUpdateItem(ptr, arrayOfGuids[i],
//                     ocrGlobalRankId, 0); ocrRouteTableFireOO(arrayOfGuids[i],
//                     ocrOutOfOrderHandler);
//                 }
//             }
//             blocksPerNode++;
...
//
//
// void ocrParallelStartReadFixedSizeLine(ocrGuid_t * arrayOfGuids, unsigned int
// numberOfGuids, unsigned int size, ocrRecordReaderLine_t reader, char *
// pathToFile, ocrMapper_t mapper, unsigned int mapperLength)
//{
//     unsigned int blocksPerNode = 0;
//     FILE * file = NULL;
...
//         }
//     }
//     struct ocrDb ** localDbArray = ocrMalloc(sizeof(struct ocrDb*) *
//     localDbArrayLength); unsigned int * localDbArrayMap =
//     ocrMalloc(sizeof(unsigned int) * localDbArrayLength*mapperLength);
...
//             {
//                 OCRSETMEMSHOTTYPE(ocrDbMemorySize);
//                 localDbArray[localOffset] = ocrCalloc(sizeof(struct ocrDb) +
//                 size); for(int j=0; j< mapperLength; j++)
//                     localDbArrayMap[localOffset*mapperLength+j] = mapper(i,
//                     j);
...
//                 if(firstRead)
//                     reader(buffer, i, &dbSize, &temp,
//                     localDbArrayMap+i*mapperLength);
//                 else
//                     reader(NULL, i, &dbSize, &temp,
//                     localDbArrayMap+i*mapperLength);
//
//                 firstRead = false;
//             }
...
//                     ocrRouteTableUpdateItem(ptr, arrayOfGuids[i],
//                     ocrGlobalRankId, 0); ocrRouteTableFireOO(arrayOfGuids[i],
//                     ocrOutOfOrderHandler); localOffset++;
//                 }
//                 blocksPerNode++;
//             }
//         }
...
// }
//
// void ocrParallelStartReadFixedSize(ocrGuid_t * arrayOfGuids, unsigned int
// numberOfGuids,
//                               unsigned int size, ocrRecordReader_t reader,
//                               char * pathToFile)
//{
...
//                     struct ocrDb * ptr = ocrCalloc(sizeof(struct ocrDb) +
//                     size); OCRSETMEMSHOTTYPE(ocrDefaultMemorySize); void *
//                     temp = ptr + 1; unsigned int dbSize = size; reader(file,
//                     i, &dbSize, &temp); ocrDbCreateInternal((void*)ptr,
//                     dbSize, DB_PROP_NONE, NULL_GUID, sizeof(struct ocrDb) +
//                     size, false); ocrRouteTableUpdateItem(ptr,
//                     arrayOfGuids[i], ocrGlobalRankId, 0);
//                     ocrRouteTableFireOO(arrayOfGuids[i],
//                     ocrOutOfOrderHandler);
//                 }
//                 else
